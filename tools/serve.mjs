 ...
 *
 * Brought over from `chunkline`, where it was written after eleven tools each
 * started their own `vite preview` on a hardcoded port and measured whatever
 * answered. This project had the milder version of the same problem and one
 * worse symptom: `tools/capture.mjs` assumed somebody had already run
 * `npm run dev` on port 5173, and died with ERR_CONNECTION_REFUSED on the
 * afternoon this repository was published.
 *
 ...
 * gate given `TOKENLAB_URL` trusted it without looking, which is the same hole in a
